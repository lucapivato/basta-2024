...
namespace Plot3D
{
	public partial class MainForm : Page
    {
        ...
        private void btnScreenshot_Click(object sender, EventArgs e)
        {
            ...
            i_Dlg.DefaultExtension = ".png";

            if (DialogResult.Cancel == i_Dlg.ShowDialog())
                return;
        ...
        private void DrawDemo()
        {
            ...
            switch (me_Demo)
            {
                ...
                case eDemo.Surface_Missing:    DemoSurface(ePolygonMode.Lines, true);  break;
        ...
        private void DemoSurface(ePolygonMode e_Mode, bool b_Missing)
        {
            ...
            cColorScheme i_Scheme = new cColorScheme(me_ColorScheme);
            cSurfaceData i_Data;
            if (b_Missing) // circles
            {
                // Constructor for circles with radius = 4 pixel. Missing points are allowed.
                i_Data = new cSurfaceData(s32_Cols, s32_Rows, 4, i_Scheme);
            }
            else // polygons / lines
            {
                ...
                // Constructor for polygons and lines. Missing points are not allowed.
                i_Data = new cSurfaceData(s32_Cols, s32_Rows, e_Mode, i_Pen, i_Scheme);
            }

            Random i_Random = new Random(3);
            int  s32_Random = 8;
            ...
            for (int C=0; C<i_Data.Cols; C++)
            {
                ...
                {
                    // In demo 'Surface Missing' omit some random points.
                    if (b_Missing && --s32_Random == 0)
                    {
                        s32_Random = i_Random.Next(7, 11);
                        continue; // omit calling SetPointAt() for this point
                    }
            ...
            if (e_Mode == ePolygonMode.Fill && !b_Missing) // Polygons
            {
            ...
            else // Lines, Circles
            {
                ...
                cPoint3D[] i_Points = new cPoint3D[] { i_Data.GetPointAt(10, 5),
                                                       i_Data.GetPointAt(10, 6),
                                                       i_Data.GetPointAt(11, 5),
                                                       i_Data.GetPointAt(11, 6) };
                foreach (cPoint3D i_Point in i_Points)
                {
                    if (i_Point != null)
