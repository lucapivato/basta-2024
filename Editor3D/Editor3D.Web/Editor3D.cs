...
namespace Plot3D
{
	...
	public class Editor3D : UserControl
    {
            ...
            public cUserInput(MouseButtons e_MouseButton, Keys e_Modifiers, eMouseAction e_Action, Cursor i_Cursor = null)
            {
                ...
                if (mi_Cursor == null)
                {
                    switch (e_Action)
                    {
                        ...
                        case eMouseAction.Theta:       mi_Cursor = Cursors.NotAllowed;  break;
                        case eMouseAction.Phi:         mi_Cursor = Cursors.NotAllowed; break;
                        case eMouseAction.Rho:         mi_Cursor = Cursors.SizeNS;      break;
                        case eMouseAction.Move:        mi_Cursor = Cursors.NotAllowed;    break;
        ...
        public class cSurfaceData : cRenderData
        {
            ...
            /// Circle mode:
            /// -------------
            /// This constructor allows that points are missing, so it is not possible to draw polygons.
            /// Lines are only drawn between existing neighbour points. If a point has no neighbour only a circle is drawn.
            /// </summary>
            public cSurfaceData(int s32_Cols, int s32_Rows, int s32_Radius, cColorScheme i_ColorScheme)
            {
                if (s32_Cols < 3 || s32_Rows < 3)
                    throw new ArgumentException("cSurfaceData needs at least 3 columns and 3 rows");

                if (i_ColorScheme == null)
                    throw new Exception("ColorScheme cannot be null.");

                mb_Circles     = true;
                ms32_Cols      = s32_Cols;
                ms32_Rows      = s32_Rows;
                mi_ColorScheme = i_ColorScheme;
                ms32_Radius    = Math.Max(s32_Radius,  2); // circles with radius = 1 would be invisible
                mi_Pen         = new Pen(Color.Yellow, 1); // color will be assigned from ColorScheme
                mi_PointArray  = new cPoint3D[s32_Cols, s32_Rows];
            }
            ...
            private void CreatePolygons()
            {
                ...
                if (mb_Circles)
                    throw new Exception("cSurfaceData: Polygons are not available in Circle mode.");
            ...
            public override void AddDrawObjects(Editor3D i_Inst)
            {
                ...
                if (mb_Fill)
                {
                    CreatePolygons(); // throws if not all points are set.

                    foreach (cPolygon3D i_Poly3D in mi_PolygonArray)
                    {
                        cPolygon i_Polygon = new cPolygon(mb_Fill, i_Poly3D, mi_Pen, mi_ColorScheme);
                ...
                if (mb_Circles)
                {
                    ...
                    if (s32_Added < 2)
                        throw new ArgumentException("At least two points must be set.");
                }
                ...
                for (int C=0; C < ms32_Cols; C++)
                {
                    for (int R=0; R < ms32_Rows; R++)
                    {
                        cPoint3D i_Start = mi_PointArray[C,R];
                        for (int V=0; V<2; V++)
                        {
                            cPoint3D i_End;
                            if (V == 0 && C+1 < ms32_Cols)
                            {
                                i_End = mi_PointArray[C+1, R];
                            }
                            else if (V == 1 && R+1 < ms32_Rows)
                            {
                                i_End = mi_PointArray[C, R+1];
                            }
                            else continue;

                            if (i_Start == null || i_End == null)
                            {
                                if (mb_Circles)
                                    continue;
                            ...
                            cLine3D i_Line3D = new cLine3D(i_Start, i_End, (int)mi_Pen.Width, mi_Pen, 1);
        ...
        private class cTooltip
        {
            ...
            public void OnMouseMove(MouseEventArgs e)
            {
                ...
                if (i_Nearest != null && s32_MinDist < i_Nearest.ms32_RadiusTip)
                {
                    ...
                    if (s_TT.Length > 0)
                    {
                        mi_Tooltip.Show(s_TT, e.X + 150, e.Y + 10);
                        return;
                    }
                }
        ...
        public Editor3D()
        {
			...
			this.Paint += Editor3D_Paint;
			this.MouseMove += Editor3D_MouseMove;
        }

		private void Editor3D_MouseMove(object sender, MouseEventArgs e)
		{
		}

		private void Editor3D_Paint(object sender, PaintEventArgs e)
		{
		}
        ...
        private void SelectionCallback(eSelEvent e_Event, Keys e_Modifiers, int s32_DeltaX, int s32_DeltaY, cObject3D i_Object)
        {
            ...
            {
                MessageBox.Show("Your callback function has crashed:\n\n" + Ex.Message + "\n\n" + Ex.StackTrace, 
